...
var defaultWeatherStyle = {"right" : 18 + "px", "left" : "auto", "top" : 7 + "px", "bottom" : "auto"};
...
function getCurrentLocation(force) {
    ...
    var breakTimeout = setTimeout(function(){
    ...
    if(!force){
        getSettingsValue("location-weather-geo", false, function(values){
            getWeatherForecast(function(weather){
                ...
                if(weather && typeof weather == "object" && typeof weather.item == "object"){ // Task #1969 // Yahoo
                    searchData = {coords:{
                        accuracy: 45,
                        latitude: weather.item.lat,
                        longitude: weather.item.long
                    }};

                    values = JSON.stringify(searchData);

                    searchData.data = {
                        LocalizedName: weather.location.city
                    }
                }
                ...
                if(!values){
                    ...
                    getLocationWeather(geo, true, searchData);
        ...
        navigator.geolocation.getCurrentPosition(function(geo) {
            ...
            getLocationWeather(geo, true, false, true);
            endEditClientPlace($("#weather-city"));
 ...
 */
function getLocationWeather(geo, skipCache, searchData, firstLoad) {
    locationWeatherCacheNeedUpdate(function(update) {
        getLastLocationWeather(function(locationWeather) {            
            ...
            if(!locationWeather) update = true;
            else if(skipCache) update = true;
            else {
                if(isAccuWeather()){/*weather on*/
            ...
            if(locationWeather && typeof locationWeather == 'object' && locationWeather.source == 'yahoo') update = true;
            ...
            if(update) {
                ...
                }else if(geo && firstLoad){//Free weather, first load
                    getPlaceWeatherAccuweather(geo, (accuweather)=>{ // Task #1988
                        geo.data = {
                            LocalizedName: accuweather.location.city
                        };
                        
                        WEATHER.openweather.getPlace(geo.coords.latitude+","+geo.coords.longitude, displayWeather, geo);
                    });
                }else{//Free weather
                    if(!locationWeather || locationWeather.source != "accuweather" || geo){
                        if(geo) {
                            WEATHER.openweather.getPlace(geo.coords.latitude+","+geo.coords.longitude, displayWeather, searchData || geo);
                        } else {
                            getCurrentLocation();
                        }
                    }else{
...
function displayWeather(locationWeather) {
    var $headerWeather = $("#header-weather");
    ...
    if(isAccuWeather() && locationWeather.source == locationWeatherSourceAccuweather){/*weather on*/
        $headerWeather.attr('weather-type', 'accuweather');
        ...
        ;
    }else if(locationWeather.source == 'openweather'){
        $headerWeather.attr('weather-type', 'openweather');

        $("#weather-icon")
        .html("")
            .addClass("openweather-icon-mini")
            .attr("title", locationWeather.weather.text ? locationWeather.weather.text.capitalizeFirstLetter() : "")
            .append(
                $("<img>")
                    .addClass("openweather-img")
                    .attr(
                        "src",
                        "./img/weather/openweather/" + locationWeather.weather.icon + ".png"
                    )
            )
            .removeAttr("data-icon")
        ;
    }else{
        $headerWeather.attr('weather-type', 'standard');
...
function addWeatherListDraggableProperty() {
    ...
    $weather.draggable({ handle: "#weather-active", containment: "#background-borders", scroll: false, delay:150,
        start: function(event, ui) {
            $weather.css('right','auto');
 ...
 * @param $el jQuery element
 */
function confirmClientPlace($el, placeByKey, fullPlace) { 
    ...
    if(val || placeByKey) {
        if(isAccuWeather()){//For premium /*weather on*/
            ...
            WEATHER.openweather.getPlace(fullPlace.data.GeoPosition.Latitude+","+fullPlace.data.GeoPosition.Longitude, displayWeather, fullPlace);
        }
...
function initLocationWeather() { 
    ...
    getLastLocationWeather(function(locationWeather) {
        getSettingsValue("location-weather-geo", false, function(geoValues){

            if(locationWeather || geoValues) {
                $intro.css({"display" : "none"});
...
function weatherAutoUpdate(){
    var period = parseInt(VAL.get('weather-auto-update'));
    var ban = parseInt(localStorage.getItem("dont-ask-weather-location")) === 1;
    
    if(isNaN(period) || !period || period == "manual" || ban) return false;
    
    period = period * 24 * 60 * 60 * 1000;
    
    var last = parseInt(VAL.get('weather-last-update')) || 0;
    var now = Date.now();
    ...
    getWeatherUnit(function(getUnit){
        ...
        getWeatherForecast(function(weather){
            ...
            for (var key in hourly) {
                ...
                var time = shortTime(date.toLocaleTimeString().replace('.',':').replace('.',':'));
