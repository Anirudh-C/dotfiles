    ...
    for(var i in groups) {
        if(groups.hasOwnProperty(i)) {
            ...
            if(group.type != GROUP_TYPE_POPULAR) {
                ...
                if(group.type == GROUP_TYPE_DEFAULT && !group.title) {
                    if(group.title)
                        $option.text(group.title);
                    else
                        $option.text(translate("page_dials_default_group_title"));
                } else 
                if(String(group.title).length > 45){
                    $option.attr("title", group.title);
                    
                    $option.text(String(group.title).substr(0, 43) + '...');
                }else{
                    $option.text(group.title);
                }
 ...
 */
function confirmMoveDialToNewGroup(editedDial, groupChanged, isNewGroup, stopProcessing) { 
    setTimeout(function() {
        BRW_sendMessage({
...
function editDial(dial, groupChanged, isNewGroup) {    
    ...
    if(isNewGroup) {
        ...
        $container.find("a.mv-tile[data-dialId=" + dial.id + "]").each(function() {
            ...
            if(previousHref != newHref || image || scheme || mode == "auto" ) {
                ...
                setTimeout(function() {
                    ...
                    if(
                        dial.title == translate("newtab_getting_title")
                        ||
                        (parseInt($url.attr("touch")) === 1 && parseInt($title.attr("touch")) !== 1)
                    ){
                        BRW_sendMessage({"command": "getAutoTitle", "dial" : dial}, (updDial)=>{
                            BRW_sendMessage(thumbLoadMessage);
                        });
