 ...
 */
    ...
    // START
    function startApplicationService() {// merge
        ...
        if (!installKey) {
            console.info("No install key");
            
            Async.chain([
                /* // Depricated for FF 57+
                function (nextHandler) { // Try to migration
                    var installTimeout = setTimeout(()=>{
                        nextHandler();
                    }, 1000);
                    ...
                    try{
                        ...
                        browser.runtime
                            .sendMessage({type:"migration", action:"log", info:ex})
                            .then(reply => {})
                        ;
                    };                    
                ...
                */
                function (nextHandler) { // Try backup
                    console.info('Try backup');
            storageBackup(
                function(){//has backup
                    console.info("Storage Backup");
                },
                function(){//no backup
                            nextHandler();
                        }
                ...
                function (nextHandler) { // App default install
                    console.info('Default install');
            ...
            console.info('Has install key');
            
            getNetTabPages(refreshNewTabPages); // Why?
    ...
    function getAvailableThemesList(callback, data) {
        ...
        //console.info(nextUpdate, nextUpdate > currentTime);
    ...
    function getFlixelThemesList(callback, data) {
        ...
        //console.item(nextUpdate && nextUpdate > currentTime);
    ...
    function getLiveSitesThumbs(mostVisitedURLs) {
        if(typeof (mostVisitedURLs) != "undefined") {
            ...
            if(tilesLength) {
                if(installWindow || installService || (liveThumbsAfterSync()) || macFullscreenWait) { // install window open
                    for (i = 0; i < tilesLength; i++) {
                        addSiteForThumbLoad(mostVisitedURLs[i]);
                    }
                } else { //install window close
					console.info(startTiles.length);
    ...
    function addSiteForThumbLoad(mv) {
        ...
        console.info("addSiteForThumbLoad", existInLoadingList, mv);

        if(!existInLoadingList)
            startTiles.push(mv);
        
        console.info("startTiles.push", "addSiteForThumbLoad", mv);
    }
	...
	function isFullScreenInterval(cb){
		var delay = 10e3;
		
		chrome.windows.getCurrent({}, (window)=>{
			if(window.state == "fullscreen"){// || window.state == "maximized"){
				console.info('It`s fullscreen, wait... ', delay);
				macFullscreenWait = true;
				
				setTimeout(()=>{
					isFullScreenInterval(cb);
				}, delay);
				
			}else{
				macFullscreenWait = false;
				console.info('Start capture');
				cb();
			}
		});			
	}
    ...
    function loadTilesThumbsInIFrame(startTiles) {		
		if(isMac/* && BROWSER == 'firefox'*/){
				isFullScreenInterval(()=>{
					loadTilesThumbsInIFrameProcess(startTiles);
				});
		}else{
			loadTilesThumbsInIFrameProcess(startTiles);
		}
    ...
    function loadTilesThumbsInIFrameProcess(startTiles) {
        ...
        var windowsParams = {
            ...
            width: 1,
            height: 1,
            type: "normal"//"popup" // Task #1534
        };
        
        if(BROWSER == 'firefox'){ // Task #1647
			if(isMac){
				//windowsParams.width = 100;
				//windowsParams.height = 100;
			}
		}else{
			windowsParams.focused = false;// Task #1534
		}
        
        var lastWindowId = getLastInstallWindow();
        
        if(lastWindowId != null) {// Task #1534			
            ...
            if(isNaN(lastWindowId)) {
				...
				if(typeof chrome.app.window == "object"){
					...
					console.info('chrome.app.window isn`t object');
    ...
    function getInstallWindow(w) {
        ...
        if(typeof (w) != "undefined") {
			...
			chrome.windows.update(w.id, winUpdateParams);			
    ...
    function loadCustomThumbImage(mv, previewItem, callback, error) {
        ...
        xhrCustomUrl.onerror = function (ex) {// merge
            console.warn(ex);
            saveSiteThumbImage(mv);
            if (callback) callback();
            //if(error) error(mv);
    ...
    function makePrintScreenOnEverhelperService(mv, callback, error) {        
        ...
        xhr.open('GET', "http://everhelper.me/sdpreviews/listing.php?p=0&order=rating&on_page=1&host=" + encodeURI(hostName) + "&" + getAddonData().join("&") + "&loadedTiles=" + loadedTiles + "&startTiles=" + startTiles.length + "&position=1", true);//https
    ...
    function makePrintScreenOnEverhelperOrLiveService(mv, callback) {
        ...
        //console.info('xhr.open', hostName);
        
        xhr.open('GET', "http://everhelper.me/sdpreviews/listing.php?p=0&order=rating&on_page=1&host=" + encodeURI(hostName) + "&" + getAddonData().join("&") + "&loadedTiles=" + loadedTiles + "&startTiles=" + startTiles.length + "&position=2", true);//https
        xhr.responseType = 'json';
        ...
        */    
        if(!checkUrlHasGoogleHost(mv.url, true)) {
            try {
                if(hostName) {
                    xhr.send();
                } else {
            ...
            if(dialId) {
                ...
                BRW_dbSelect({ //Param
                    ...
                    function (results) { //Success
                        ...
                        setTimeout(function() {
                            getNetTabPages((mvData)=>{// merge
                                sendThumbToApp(mvData);
                            }, {"mv" : mv});
                        }, 250);
                    },
                    function (error) { //Error
    ...
    function sendThumbToApp(data) {
        if(BROWSER != "chrome"){// merge
            
            BRW_sendMessage({"command" : "tileThumbLoadComplete", "mv" : objClone(data.mv)}); //For firefox
        }
        else{//For chrome
            var tabs = data.tabs;
            var tabsCount = tabs.length, i;
            for(i = 0; i < tabsCount; i++){// merge
    ...
    function printScreenOutsideService(){
        //if(BROWSER && BROWSER == 'firefox') return true; // Task #1380
        return false;
    }
