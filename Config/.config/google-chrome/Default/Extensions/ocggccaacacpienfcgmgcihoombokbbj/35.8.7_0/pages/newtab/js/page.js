    ...
    function checkMenuActionsHandler(){
        if(browserName(true) != "ff" || true) return false;// merge
        //BRW_sendMessage({"command": "checkContextMenu"}); // FF Migration
    }//function
...
$(()=>{
    ...
    $("body").attr("platform", platform.indexOf( "win" ) === 0 ? "win" : (platform.indexOf( "mac" ) === 0 ? "mac" : (platform.indexOf( "linux" ) !== -1 ? "linux" : "other")));
    
    if(/\bCrOS\b/.test(navigator.userAgent)){// Task #1556
        $("#header").addClass('chromeos');
    }
    
    BRW_sendMessage({"command": "increaseThemeStatisticTimer"});
});