...
function reloadDialThumb($el, action, thumbType) { 
    if ($el) {
        ...
        BRW_sendMessage({"command": "getAutoTitle", "dial":{id:dialId, url:mvUrl}}, (updDial)=>{ // Task #1733
            console.info(updDial);
        });
    }
}
...
function getTileContextMenuItems(showEditItem, isFile) { 
    ...
    items.push({
        text: translate("page_dial_context_menu_open_new_window_thumb"),
        alias: "openNewWindow",
        action: function (el) {
            openDialUrl($(el), openNewWindow);
        }
    });
    items.push({
...
function initTilesContextMenu(group, forceEdit) { 
    ...
    $(document).on('contextmenu', ".mv-tiles .mv-new-dial", function(e) {
        e.preventDefault();
    }); 
