        ...
        query   : {
            all : {
                ...
                parser : (RAW)=>{
                    ...
                    console.info(RAW.query.results.place);
...
function ModSuggestions(Data){
    ...
    this.setPlace =()=> {
        ...
        if(typeof confirmClientPlace == "function") confirmClientPlace(UI.$input, ts.state.cursor.key, List.parse[ts.state.cursor.key]);
        else
        if(typeof confirmClientPlaceSettings == "function") confirmClientPlaceSettings(UI.$input, ts.state.cursor.key, List.parse[ts.state.cursor.key]);
    ...
    this.setApi =()=>{
        //if(ts.state.query.pro && AUTH.isPremium()){  // Task #1969
        if(true){
