var fkSuccess = function(server_prod_deets_var, fetch_url, chk_quality = false, back_end_response_deets = {}) {
    ...
    return function(response, textStatus, sent_req) {
        ...
        insert_price_result_box(make_results_box({
            "prod_site": "fk",
            "prod_link": prodlink_href,
            "website": "Flipkart",
            "title": prodlink_txt,
            "img_src": dyn_prod_img,
            "prod_price": parseInt(price_str)
        }, "searchid"));
...
var sdSuccess = function(server_prod_deets_var, fetch_url, chk_quality = false, back_end_response_deets = {}) {
    ...
    return function(response, textStatus, sent_req) {
        ...
        var sd_ass = {
            'srchdiv': '#products div.product-tuple-listing:eq(0)',
...
var azSuccess = function(server_prod_deets_var, fetch_url, chk_quality = false, back_end_response_deets = {}) {
    ...
    return function(response, textStatus, sent_req) {
        ...
        if ((!avail_val) || (resp_elem.find('#noResultsTitle').length > 0)) {
            ...
            if (prodlink_txt == '') {
                prodlink_txt = $.trim(resp_elem.find('#atfResults #result_0 h2.s-access-title').text());
            }
            price_str = $.trim(price_str.replace(/rs\.*|\*|\,/gi, ''));

            console.log("az back search prod link text");
            console.log(prodlink_txt);

            console.log((prodlink_txt.startsWith("[Sponsored]") || prodlink_txt.startsWith("[sponsored]")));

            if(prodlink_txt.startsWith("[Sponsored]") || prodlink_txt.startsWith("[sponsored]")){
            	var results = resp_elem.find("#s-results-list-atf li.s-result-item");
            	console.log("searching for non sponsored reuslt")
            	for(var i=0; i<results.length; i++){
            		var result = $("<div/>").append(results[i]);
            		var title = result.find("h2.s-access-title").text();
            		console.log(title);
            		if(title.startsWith("[Sponsored]") || title.startsWith("[sponsored]")){
            			continue;
            		}
            		prodlink_href = result.find('a.s-access-detail-page').attr('href');
            		prod_img = result.find('img.s-access-image').attr('src');
            		prodlink_txt = result.find('h2.s-access-title').text();
            		price_str = result.find('.s-price').text();
            		price_str = $.trim(price_str.replace(/rs\.*|\*|\,/gi, ''));

            		// 1st non sponsored result found
            		break;
            	}
...
var mySuccess = function(server_prod_deets_var, fetch_url) {
    ...
    return function(response, textStatus, sent_req) {
        ...
        if ((!avail_val)) {
            ...
            var myn_ass = {
                'price': 'div.results-cnt ul.results li[data-styleid]:eq(0) a div.price',
    ...
    return function(response, textStatus, sent_req) {
        ...
        if (avail_val) {
            ...
            if (resp_elem.find(".notifyme-bttntxt").length > 0) {
                ...
                insert_oos_box(make_results_box({
                    "prod_site": "ba",
                    "prod_link": prodlink_href,
                    "website": "BookAdda",
                    "title": prodlink_txt,
...
var tcSuccess = function(server_prod_deets_var, fetch_url, chk_quality = false, back_end_response_deets = {}) {
    return function(response, textStatus, sent_req) {
        var results_json = response;
        console.log(results_json);
        if(results_json.searchresult && results_json.searchresult.length>0){
            var first_result = results_json.searchresult[0];

            var prodlink_href = "";
            var prodlink_txt = "";
            var prod_img = "";
            var price_str = "";
            if(first_result.webURL){
                prodlink_href = first_result.webURL;
                if(!prodlink_href.startsWith("http")){
                    prodlink_href = "https://www.tatacliq.com"+prodlink_href;
                }
            }
            if(first_result.productname){
                prodlink_txt = first_result.productname;
            }
            if(first_result.imageURL){
                prod_img = first_result.imageURL;
                if(!prod_img.startsWith("http")){
                    prod_img = "https:" + prod_img;
                }
            }
            if(first_result.price && first_result.price.sellingPrice && first_result.price.sellingPrice.doubleValue){
                price_str = first_result.price.sellingPrice.doubleValue;
            }

            console.log(prodlink_href,prodlink_txt,prod_img,price_str);

            if(prodlink_href && prodlink_txt && prod_img && price_str ){
                insert_price_result_box(make_results_box({
                    "prod_site": "tc",
                    "prod_link": prodlink_href,
                    "website": "TataCliq",
        ...
        insert_manual_search_box(make_manual_search_box({
            "prod_site": "tc",
            "prod_link": "https://www.tatacliq.com/search/?searchCategory=all&text="+encodeURI(prod_deets.prod_title),
            "website": "TataCliq",
            "title": prod_deets.prod_title,
            "img_src": prod_deets.prod_img
        }, "searchid"));
