...
if (window.location.hostname == 'www.tatacliq.com') {
	...
	var tc_sub_kp=false;
	var tc_sub_kp_sub_val='';

	$("body").on("keydown","#search_form input[type='text']",function(){
		if(event.which == 13) {
			// alert($(this).val());
			sendsearchIntent('tc',$(this).val());
	      	tc_sub_kp=true;
	      	tc_sub_kp_sub_val=$(this).val();

			return true;
		}
	});

	$("body").on("click",".ui-autocomplete li a",function(){
			// alert($(this).text());
			sendsearchIntent('tc',$(this).text());
			return true;

	});

	$("body").on("click","#search_form #searchButton",function(){

			if(tc_sub_kp && tc_sub_kp_sub_val == $("#search_form input[type='text']").val()) {
	  			tc_sub_kp=false;
	  		}
	  		else{
				sendsearchIntent('tc',$("#search_form input[type='text']").val());
				return true;
	  		}
			// alert($("#search_form input[type='text']").val());

	});

	$("body").on("click","#buyNowButton",function(){
			// alert("Buy Button Clicked");
			sendcartevent('tc','buy button',prod_deets.product_id,window.location.href,prod_deets.prod_title);
			return true;
	...
	$("body").on("click","#addToCartButton",function(){
			...
			sendcartevent('tc','add to cart',prod_deets.product_id,window.location.href,prod_deets.prod_title);
			return true;
