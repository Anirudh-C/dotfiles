...
(function() {    
                            ...
                            if(imagesResultsLength) {
                                for (var i = 0; i < imagesResultsLength; i++) {
                                    ...
                                    if(typeof imageObj == "object"){
                                        tile.thumbType = 2;
                                        loadCustomThumbImage(tile, imageObj, false, (ex)=>{console.warn(ex);}); // merge
                                    }else{
    ...
    function updateDialThumbByDefaultType(tiles) {
        getNewSpeedDialThumbType(function(newDialThumbType) {
            if(tiles && tiles.length == 1){
                //if(isAmazonProductURL(tiles[0].url)) newDialThumbType = speedDialThumbTypeAutopreviewText; // Depricated
            }
    ...
    function loadRandomBackgroundFile(fs, sendResponse, defaultResponse) {
        //console.info('loadRandomBackgroundFile', Date.now());
                ...
                BRW_fsGetFile(fs, filePath, 
                    ...
                    function(error){//File NOT found
                        ...
                        */
                        ...
                        var command = {
                            theme: favoriteInstalledTheme.id,
                        };
                    
                        //console.info(filePath, favoriteInstalledTheme);
                    
                        if(favoriteInstalledTheme.lastInstallBgVideo){
                            command.resolution = favoriteInstalledTheme.lastInstallBgVideo ? favoriteInstalledTheme.lastInstallBgVideo.resolution : false;

                            if(favoriteInstalledTheme.isFlixelContent){
                                command.func = changeBackgroundFlixerVideoFile;//changeFlixerVideoBackground;
                            }else{
                                command.func = changeBackgroundVideoFile;
                            }
                        }else{
                            command.func = changeBackgroundImageFile;
                        }
                    
                        try{
                            localStorage.setItem("dont-reload-tabs", "soft:updateBackgroundSoft");

                            getFileSystem(command.func, {themeId: command.theme, resolution: command.resolution || false, sendResponse: (response)=>{
...
if (BROWSER == "chrome") {
    ...
    function chromeTabsChange(reason) {
        ...
        setTimeout(function () {
            ...
            chrome.tabs.query({currentWindow: true}, function (Tabs) {// merge
    ...
    if(!localStorage.getItem("addon-install-version")) localStorage.setItem("addon-install-version", chrome.app.getDetails().version);
    ...
    chrome.management.getSelf((addonData)=>{
        localStorage.setItem("addon-version", addonData.version);
        
        if(!localStorage.getItem("addon-install-version")) localStorage.setItem("addon-install-version", addonData.version);
