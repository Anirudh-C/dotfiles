...
function FocusLSP(){
    ...
    this.newtabInit =()=> {
        ...
        ts.state.icoMargin = String((parseInt(ui.newtab.lineHeight) - parseInt(ui.newtab.iconSize)) / 2) + "px";
    ...
    this.coordinates =(safe, read)=> {
        ...
        if(ts.coord.el.w > ts.coord.win.w - 100){
            //ts.coord.el.w = ts.coord.win.w - 170;
            
            var maxWidth = ts.coord.win.w - 270;
            ...
            ui.newtab.$text.addClass('white-space');
                
            console.info('fired');
            
            ts.coord.el.w = 4 * (parseInt(ts.state.icoMargin) || 10) + 52 + maxWidth;//parseInt(ui.newtab.$wrap.width());
        }
    ...
    this.focusMode =(mode, done)=> {
        ...
        if(mode == true){
            ...
            ui.newtab.$header.addClass("focus-header-active").text(translate('focus_title'));
        }else{
            ...
            ui.newtab.$header.addClass("focus-header-active").text(translate('focus_question'));
        }//else
