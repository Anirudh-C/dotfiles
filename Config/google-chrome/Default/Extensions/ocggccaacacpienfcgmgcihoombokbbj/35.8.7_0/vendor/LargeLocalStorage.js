...
function createPipeline(pipedMethodNames) {
	...
	pipedMethodNames.forEach(function(name) {
		...
		nextMethods[name] = createPipelineNext(name);
            /* CSP Error, need unsafe-eval
            new Function(
		...
		pipeline[name] = createPipelineFunction(name);
            /* CSP Error, need unsafe-eval
            new Function(
    ...
    function createPipelineNext(name){
        var func = function(){
    ...
    function createPipelineFunction(name){
        var func = function(){
            //console.info('Call', name);
            var ctx = new this.pipe._contextCtor(this.pipe._handlers, this.pipe._nextMethods[name], this.pipe.end);
            return ctx.next.apply(ctx, arguments);
