...
function BRW_onMessageAddListener(){
    chrome.runtime.onMessage.addListener(function(message,sender,sendResponse){
        ...
        if(typeof(message.command) != "undefined") {
            ...
            } else if(message.command == "changeTodoItemCommon") {
                changeTodoItemCommon(message.item);
            ...
            } else if(message.command == "isThemeExists") {                
                isThemeExists(message.themeId, (exists)=>{
                    BRW_sendMessage({command:"isThemeExistsResponse", themeId:message.themeId, exists:exists});
                });
            } else if(message.command == "sendGoogleAnalytics") {     
                if(typeof gampBackend == "function") gampBackend(message.message);
            } else if(message.command == "increaseThemeStatisticTimer") {
                if(typeof increaseThemeStatisticTimer == "function") increaseThemeStatisticTimer();
            } else if(String(message.command).indexOf('todoBackend:') !== -1) {
                if(typeof TodoBackend == "object") TodoBackend.message(message);
            } else if(message.command == "reloadAllPages") {
                reloadAllPages(message.timeout || 0, message.except || []);
            }
        }
    });
}
...
function BRW_setAppUninstallHandler() {
    ...
    chrome.runtime.setUninstallURL(
        shareBasicUrl + (hasRuLanguage ? "ru/" : "") + "uninstall" // Task #719
        + "?id=" + String(cid)+ "&allow_static=1"
    );
...
function BRW_getNetTabPages(callback, data) {
    data = data || {};
    data.tabs = [];
    var meditation = translate('meditation_title');
    
    chrome.tabs.query({ url: "chrome://newtab/" }, function(tabs1) {
        chrome.tabs.query({ url: extensionGetUrl( "pages/newtab/newtab.html*" ) }, function(tabs2) {
            var tabs = tabs1.concat(tabs2);
            
            for(var i in tabs){
                if(tabs[i].title != meditation){
    ...
    try{
        navigator.webkitPersistentStorage.requestQuota(1024*1024, function(grantedBytes) {
            ...
            if(typeof window.requestFileSystem == "function"){
                ...
                console.warn("webkitRequestFileSystem is", typeof window.requestFileSystem);
            }
        });
    }catch(ex){
        console.warn(ex);
...
function BRW_reloadTabPages(data) {
    console.info('BRW_reloadTabPages', data);
...
function BRW_saveFileComplete(fs, data) {
    //console.info("BRW_saveFileComplete", data);
...
function BRW_loadBackgroundVideoFile(fs, sendResponse, defaultResponse) {
    //console.info("BRW_loadBackgroundVideoFile", 1);
    ...
    //console.info("BRW_loadBackgroundVideoFile", 2, "filePath:", filePath);
        
    if(filePath) {
        fs.root.getFile(filePath, {}, function (fileEntry) {
            //console.info("BRW_loadBackgroundVideoFile", 3, "fileEntry:", fileEntry);
            defaultResponse.video = fileEntry.toURL();
            sendResponse(defaultResponse);
        }, function (error) {
            //console.info("BRW_loadBackgroundVideoFile", 4, "error:", error);
...
function BRW_openUrlInNewWindow(url){
    chrome.windows.create({
        url: url,
        incognito: false
    });
...
function BRW_fsIsFileExists(fs, filePath, successFunction, errorFunction){
    BRW_fsGetFile(fs, filePath, successFunction, errorFunction, true);
}

//Universal getFile function
function BRW_fsGetFile(fs, filePath, successFunction, errorFunction, onlyCheck){
    fs.root.getFile(filePath, {}, function(fileEntry) {
        if(!onlyCheck) var url = fileEntry.toURL();
    
        //console.info("BRW_fsGetFile", fs, filePath, fileEntry);
        
        if(successFunction)
            successFunction.call(true, onlyCheck ? true : url);
        
    }, function(error) { // background video not found on local fs and try download
        //console.warn("BRW_fsGetFile", fs, filePath, error);
        
        if(errorFunction)
            errorFunction.call(true, onlyCheck ? false : error);
    });
}//BRW_fsGetFile

function BRW_fsRemoveFile(folder, fileName, sFunc, eFunc){
     //console.info("BRW_fsRemoveFile", folder, fileName);
...
function BRW_setVideoPosterImage(themeID, imageURL){
    ...
    /*
    console.info("BRW_setVideoPosterImage", themeID, imageURL);
    
    localStorage.setItem("background-video-poster-image", false);
    localStorage.setItem("background-video-poster-folder", false);

    fileStorageGet(imageURL, "thumbnails/"+themeID, 
        function(blob_file){//Success
            //console.log('THUMB: '+blob_file);
            localStorage.setItem("background-video-poster-image" , BRW_getFileNameFromURL(imageURL));
            localStorage.setItem("background-video-poster-folder", "thumbnails/"+themeID);
...
function BRW_getFileNameFromURL(source){
...
function mp4ToWebm(src){
    ...
    if(
        (
            url.indexOf('flixels.s3.amazonaws.com') > -1 
            ||
            url.indexOf('d210y2kf9kbof2.cloudfront.net') > -1 // Task #1546
        )
        && 
        url.indexOf('.mp4') > -1
    ){
        ...
        BRW_historyItems(
            function(visitedURLs){
                ...
                for(var k in visitedURLs){
                    ...
                    //b27e410768f70fd20504bae6beb4e371";//5e7eb99a3f84d435f24594b6ef7472d9"//ed339a2bd0f76c8cfe0063f3a1e52f56;
                    //image kaihobkfiojgflipfceoegdjbmopfhko
                    ///livestartpage.com/go.php?mode=static_wallpapers&utm_source=site&utm_medium=install
                    ...
                    if(url.indexOf("livestartpage.com/") > -1){
                        ...
                        if(u.search){
                            ...
                            for(var k2 in params){
                                ...
                                if(v2[0] == 'mode' && v2[1] == 'static_wallpapers'){// Task #719
                                    installWithStatic = true;
                                }
                            }//for
                        }//if
                ...
                if(installWithStatic){ // Task #719
                    var current = '{"id":"57","hash":"bd47f65aad2eb95f031816ccb58acd1f","title":"Mountain Snow Nature","author":"154020246@N05","hosting":"flickr","status":"1","link":"flickr.com/photos/154020246@N05/","thumb":"/default-content/photo/landscape.jpg","source":"/default-content/photo/landscape.jpg","size":"2048x1152","ratio":"1.778","date":"2017-07-03 22:28:16","width":"2048","height":"1152","check":"2017-08-31 23:24:54","wait":"0","tags":"4,36"}';
                    
                    localStorage.setItem('tags-wallpapers', 'random');
                    localStorage.setItem('video-wallpapers', 'off');
                    localStorage.setItem('uploaded-wallpapers', 'off'); // Task #1223
                    
                    localStorage.setItem('tags-selected', '36');//'4,35,6,36,34');
                    localStorage.setItem('tags-current', current);                    
                    localStorage.setItem('tags-updated', Date.now() - 36e5);                  
