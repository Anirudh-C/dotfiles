function BRW_getDb(){
    if(!appDB) {
        ...
        appDB.transaction(function (tx) {
            ...
            createColumnsIfNotExist(tx, "TODO_ITEMS", ["due"]);
...
function BRW_setDefaultGroup(callback) {
    ...
    GROUP_DEFAULT_ID = generateUniqueId(addDate);
            ...
            tx.executeSql('SELECT * FROM GROUPS ORDER BY orderNum', [], function (tx, results) {
                ...
                if(results && results.rows){
                    for(var key in results.rows){
                        ...
                        if(val && val.id && gTitle){
                            orderNum = Math.max(orderNum, parseInt(val.orderNum));
                            
                            var gTitleCut = gTitle.split('-');
                            var inc = gTitleCut.pop().trim();
                            gTitleCut = gTitleCut.join('-').trim();
                            
                            if(gTitle == title && nameStep == 0) nameStep = 1;
                            else if(
                                (gTitle.length > title.length )
                                && 
                                (gTitle.indexOf(title) === 0 )
                                && 
                                (gTitleCut == title)
                            ){
                                //var inc = String(gTitle.split(title)[1]).replace('-', '').trim();
                                
                                if(inc && Boolean(parseInt(inc)) && String(parseInt(inc)) == String(inc) && parseInt(inc) >= nameStep){
