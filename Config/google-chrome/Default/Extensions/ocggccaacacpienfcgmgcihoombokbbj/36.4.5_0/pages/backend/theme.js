 ...
 */
    ...
    function getThumbImageElementsUrl(data) {
        //console.info("getThumbImageElementsUrl", data);
    ...
    function tryGetBackgroundVideoFile(fs, data) {
        //console.info("tryGetBackgroundVideoFile", data);
        ...
        fs.root.getFile(data.filePath, {}, function(fileEntry) {
            //console.info("Is exists", data.filePath);
            ...
            console.warn("Not exists", data.filePath);
     ...
     * @param callback Function
     */
    function loadFlixerThemeConfig(themeId, callback, params, NoUpdate) {
    ...
    function getThemeConfig(themeConfig, data) {
        //console.info("getThemeConfig", data);
...
/**
    ...
    function tryGetBackgroundImageFile(fs, data) {
        //console.info("tryGetBackgroundImageFile", data);
        ...
        fs.root.getFile(data.filePath, {}, function(fileEntry) {
            //console.warn("Is exists", data.filePath);
    ...
    function increaseThemeStatisticTimer() {
        if(localStorage.getItem('video-wallpapers') != "on") return false;
        
        var lastThemeStat = parseInt(localStorage.getItem('last-theme-stat')) || 0;
        var now = Date.now();
        var period = 24 * 60 * 60 * 1000;
        
        if(now - lastThemeStat <= period) return true;
        
                
        var currentThemeId = getThemeIdByFilePath(getStorageBackgroundVideoFile());
        var currentResolution = getBackgroundVideoFileResolution();
        var contentType = getBackgroundVideoContentType();
        var installedThemes = getInstalledThemes();
        
        var theme = installedThemes[currentThemeId];
        
        //console.info(contentType, currentThemeId, currentResolution, theme);
        
        increaseThemeStatistic(contentType, currentThemeId, currentResolution, theme);
    ...
    function increaseThemeStatistic(bgType, contentId, resolution, theme) {
        var send = true;
        //console.info('increaseThemeStatistic', bgType, contentId, resolution, theme);
        ...
        if(send){
            localStorage.setItem('last-theme-stat', Date.now());
    ...
    function isThemeExists(themeId, callback){
        var themes = JSON.parse(localStorage.getItem("installed-themes"));
        
        if(themes[themeId]){
            var theme = themes[themeId];
            
            var dir = getThemesFileSystemPath(themeId);
            
            //console.info(theme); console.info(dir, file);
            
            if(theme.lastInstallBgVideo) var file = theme.lastInstallBgVideo.fileName;
            else var file = String(getThemeFileSystemPath(theme.bgFilePath[getArrayFirstKey(theme.bgFilePath)], theme.id)).replace(dir + '/','');
            
            getFileSystem(function(fs, data) {
                BRW_fsIsFileExists(fs, dir + "/" + file, 
                    (exists)=>{
                        //console.info("response", exists);
                        callback(exists);
                    },
                    (error)=>{
