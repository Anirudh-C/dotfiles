 ...
 */
    ...
    function howToUninstallDialogHandler() {
        var $link = $("#how-to-uninstall");
        
        $link.on("click", ()=>{
            
            var message = translate("how_to_uninstall_text");
            
            switch(browserName()){
                case "firefox":
                    message = message.replace('[link]', 'about:addons');
                break;
                   
                case "opera":
                    message = message.replace('[link]', 'chrome://startpage/extensions');
                break;
                    
                default:
                    message = message.replace('[link]', 'chrome://extensions/');
                    
            }
            
            dialogConfirm({
                title   : translate("how_to_uninstall_link"), 
                message : $("<p>").addClass('font-18').html(message), 
                confirmTxt:"OK", 
                cancelHide:true
            });
        });
    ...
    function relaxAutoShuffleHandler() {
        ...
        var current = parseInt(localStorage.getItem("relax-sidebar-shuffle-interval")) || 30000;
    ...
    function displayClockFormat() {
        ...
        var $clockFormatLabel = $("#clock-format-label-block");// merge
        ...
        if($currentEl.length) {
            $currentEl.each(function() {
                ...
                // merge
                $clockFormatLabel.removeClass("enabled disabled").addClass(itemVal ? "enabled" : "disabled");
            });
        }
    ...
    function displayClockVisibleLabel() {
        ...
        $el.on("change", function(e) {
            ...
            //console.info(val, $clockVisibleLabelColor.is(":visible"));
    ...
    function displaySearchFormBlock() {
        ...
        waitAuth(function(){// Task #2026
            if(AUTH.isPremium()){// Task #2026
                $('#search-form-switcher').removeClass('hidden');

                //Task #2047
                $('.premium-only-search').removeClass('hidden');
            }
            
            getDisplaySearchForm(function(display) {
                if(!AUTH.isPremium() && !display){
                    display = true;
                }
            ...
            waitAuth(function(useAuth){                
                ...
                if(true){
                    ...
                    getSearchFormProviderType(function(searchPorviderType) {
                        //console.info("searchPorviderType", searchPorviderType);
                        
                        $(".search-form-provider-item[data-search-provider=" + searchPorviderType + "]").each(function(){
                            var $item = $(this);
                            
                            if(parseInt($item.attr("data-search-provider")) === parseInt("searchPorviderType")){
                                $item.attr("checked", "checked").trigger("click");
                            }else{
                                $item.attr("checked", "");
                            }
                        });
                        ...
                        setTimeout(()=>{
                            $(".search-form-provider-item[data-search-provider=" + searchPorviderType + "]").trigger("click")
    ...
    function displayClockColorBlocksItem(clockType, animate) {
        ...
        //console.info(lableIsVisible);
        
        switch (clockType) {
            case 3:
            {
                if(lableIsVisible) $colorLabel.removeClass("disabled").addClass("enabled");//show();// OLD > New Settings
