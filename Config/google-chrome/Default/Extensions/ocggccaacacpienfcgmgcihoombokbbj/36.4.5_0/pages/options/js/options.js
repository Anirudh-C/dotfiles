...
function getAvailableThemes(response, callback, interest) {
    ...
    //var isDownloaded = (NAVI.state.page.sub === "navi-bg-downloaded" && (naviTabs.length === 0 || naviTabs.indexOf("downloaded-live-backgrounds") !== -1)); // Task #719
    var isDownloaded = NAVI.isTabActive("downloaded");
    ...
    if (availableThemesData) {
        ...
        //if (imagesActiveTab == 1 || typeof NAVI == "object" && NAVI.state.page.sub == "navi-bg-downloaded") { //by user // Task #719
        if (imagesActiveTab == 1 || NAVI.isTabActive("downloaded")) { //by user
        ...
        $("#uploaded-wallpapers-on").attr("disabled", themesList["static_backgrounds_user"].length ? false : "disabled");// Task #1223
    }
    ...
    if (flixelThemesData) {
        ...
        if(isDownloaded){ // Task #851
            //console.info(flixelThemes);
            
            var ids = [], size=0;
            for (i in flixelThemes){
                size++;
                ids.push(flixelThemes[i].id);
            }

            for (i in response.installedThemes) {
                if(typeof response.installedThemes[i].bgVideoPath == "object" && Object.keys(response.installedThemes[i].bgVideoPath).length){
                    if(ids.indexOf(response.installedThemes[i].id) === -1){
                        var contentItem =  response.installedThemes[i];

                        contentItem['contentType'] = flixelBackgroundType;

                        //if(!contentItem.isFlixelContent) contentItem.handmade = true;
                        ...
                        if(!contentItem.handmade) {
                            ...
                            contentItem = commonAddFlixelHdVideoIfNotSetForFullHd(contentItem);
                        } else {
                            if(!contentItem['bgFileThumb'])
        ...
        for (i in themesList["flixel_backgrounds"]) {
            ...
            /*
            console.info(themesList["flixel_backgrounds"][i]);
            console.info(isLiveTheme);
            */
...
function getAvailableThemesFavorites(response, callback) {
    //console.info("getAvailableThemesFavorites", response);
                ...
                if ( /*contentType && */ contentId) {
                    ...
                    if (true || Object.keys(themes).length > 1) { //Delete theme
                        if (themes[contentId]) {
                            ...
                            //else if (typeof NAVI == "object" && NAVI.state.page.sub == "navi-bg-downloaded") { // delete block from downloaded // Task #719
                            else if (NAVI.isTabActive("downloaded")) { // delete block from downloaded
...
function addAvailableContentToList($container, theme, contentType, currentContent) { //return;
    //console.info("addAvailableContentToList", theme);
    //console.trace();
    ...
    try {
        if (typeof VAL === "object" && typeof NAVI === "object") {
            ...
            //if (section === "navi-bg-favorites") {} // Task #719 ts.isTabActive("favorites")
            if (NAVI.isTabActive("favorites")) {}
            else
            //if (section === "navi-bg-downloaded") { // Task #719
            if (NAVI.isTabActive("downloaded")) {
    ...
    if (contentType == liveBackgroundType) {
        ...
        if (isInstalledContent) {
            if (isActiveContent) {
                ...
                $viewItem.parents(".av-content-container").removeClass("not-exists");
        ...
        */
        ...
        //if (isInstalledContent && !isActiveContent && getRouteSortType() != 3 && section != "navi-bg-favorites" && section != "navi-bg-downloaded") return; // Why? // Task #719
        if (isInstalledContent && !isActiveContent && getRouteSortType() != 3 && section != "navi-bg-favorites-and-downloaded") return; // Why?
...
function getAvailableContentError(message) {
    jGrowlApply(translate("options_refresh_themes_list_error"), {
        "life": 10000
    });
}
/**
...
function changeBackgroundImageFileError(message) {
    ...
    jGrowlApply(translate("options_tabs_download_static_bg_error"));
...
function changeBackgroundVideoContentError(message, contentType) {
    ...
    if (contentType == flixelBackgroundType) jGrowlApply(translate("options_tabs_download_flixer_bg_error"));
    else jGrowlApply(translate("options_tabs_download_live_bg_error"));
...
function changeStaticBackgroundToOptionsPage(message) {
    ...
    //checkDisplayRandomThemesActive(); Task #1512
    VAL.set("uploaded-wallpapers", "on"); // Task #1512
...
function changeVideoBackgroundToOptionsPage(message, contentType) {
    ...
    //checkDisplayRandomThemesActive(); // Task #1512
    VAL.set("video-wallpapers", "on"); // Task #1512
...
function addInstallResolutionLiveBackgroundHandler() {
    $(document).on("click", ".current-theme-download", function (e) {
        ...
        if($el.hasClass("btn-success")) return true;
...
function addImagesUploadButtonHandler() {
    ...
    $uploadApply.on('click', function (e) {
        ...
        if (formData.title && ((formData.url && checkImageURL(formData.url)) || formData.file)) {
            ...
            VAL.set("randomize-downloaded", {value: 'off', apply:true, nogo:true});
            VAL.set("randomize-favorites", {value: 'off', apply:true, nogo:true});
        }
...
function downloadImageByUserComplete() {
    ...
    if($uploadDialog.css('display') !== "block") return false;
...
function addDisableVideoButtonHandler() {
    $(document).on("click", "#disable-current-video, #disable-current-flixel", function () {
        BRW_sendMessage({
            ...
            //if (isNaN(clockType)) setClockColorScheme(getClockColorsObject()); // Task #1277
...
function updateAvailableThemesListOnPageLoad() {
    ...
    var $waiter = $(".navi-themes").html("");
    
    var naviTabs = NAVI.state.page.tab || [];
    var isSearchThemes = (naviTabs.indexOf("live-bg-search") !== -1);
    if(!isSearchThemes) $waiter.addClass("wait-for-themes");
...
function themesSearch(data) {
    ...
    myself.actions = {
            "themes": {
                ...
                hintUrl: "http://livestartpage.com/api/getComplexFlixelThemes?tsort=1&count=6&top=1&flixel_status_ignore=1&time={time}&search={query}"
                , parser: function (r) {
                    return JSON.parse(r);
                }
    ...
    this.init = function () {
        ...
        , this.modalHint = function () {
            ...
            if (!query.length) { //reset datalist
                ...
                myself.timeout = setTimeout(()=>{
                    myself.modalHintQuery(query);
                }, 500);
            }
        }
        , this.modalHintQuery = function (query) {
            myself.last = query;

            //if (myself.xhr) myself.xhr.abort();

            myself.queryId = Date.now();
            var url = myself.current.hintUrl.replace('{time}', myself.queryId).replace('{query}', encodeURIComponent(query)).replace('{lang}', encodeURIComponent(navigator.language));

            var xhr = BRW_ajax(url, 
                function (response) {
                    var searchResponse = xhr.responseText;
                    searchResponse = myself.current.parser(searchResponse);

                    if(searchResponse.time != myself.queryId){
                        return false;
                    }
...
function activateWallpapers(mode){
    if(mode == "uploaded"){
        if(String(VAL.get('background-image-file')).indexOf('usr') !== 0){
            $("#available-image-themes .available-install-static:eq(0)").trigger('click');
        }
    }else
    if(mode == "video"){
        if(!VAL.get('background-video-file')){
            var themes = getInstalledThemes();
            
            for(var i in themes) if(typeof themes[i].lastInstallBgVideo == "object"){
                BRW_sendMessage({
                    command: themes[i].isFlixelContent ? "changeFlixerVideoBackground" : "changeVideoBackground"
                    , theme: themes[i].id
                });
