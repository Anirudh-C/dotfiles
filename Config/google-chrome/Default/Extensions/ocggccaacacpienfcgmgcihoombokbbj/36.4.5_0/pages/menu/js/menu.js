...
function PanelMenu(){
    ...
    self.obj = {
        ...
        li  : {
            ...
            $meditation  : $("#menuPanelMeditation"),
            $themes      : $("#menuPanelSelectTheme"),
            $themesVideo : $("#menuVideoThemes"),
            $themesStatic: $("#menuStaticPictures"),
    ...
    this.init = function(){
        ...
        if(locked && locked == "on") self.closePanel();
        ...
        self.obj.li.$newtab.unbind("click").on("click", function(e){
            ...
            self.closePanel(); // Task #1303
        });

        self.obj.li.$meditation.unbind("click").on("click", function(e){
            e.preventDefault(); e.stopPropagation();
            openUrlInNewTab(extensionGetUrl("/pages/newtab/newtab.html#meditation"));
            //openUrlInCurrentTab(extensionGetUrl("/pages/newtab/newtab.html#meditation"));
        ...
        self.obj.li.$themesVideo.unbind("click").on("click", function(e){
            e.preventDefault(); e.stopPropagation();
        ...
        self.obj.li.$themesStatic.unbind("click").on("click", function(e){
            e.preventDefault(); e.stopPropagation();
            openUrlInNewTab(extensionGetUrl("/pages/options/options.html#navi-bg-wallpapers"));
    ...
    this.setAsNewTab = function(){
        if(
            (head.browser.name == 'ff' && false)// merge
    ...
    this.setPanelSize = function(){
        ...
        var height = $("#menuPanel").css("display") !== "none" ? $("#menuPanel").height() + 5 : $("#saveSomeTabsWrap").height() + 28 // Task #1302;
    ...
    this.getDials = function(){
        if(typeof getLocalStorage == "function"){
            getLocalStorage("tiles-block-display", self.getDialsDisplay);
        }else{
            self.getDialsDisplay(localStorage.getItem("tiles-block-display"));
        }//else
    };
    
    this.getDialsDisplay = function(value){
        //self.obj.some.$dials.text(Date.now() + " | " + value + " / " + parseInt(value) + " / " + (parseInt(value) !== 0));
        
        if(value === undefined || parseInt(value) !== 0){
            self.obj.some.$dials.removeClass("hide");   
        }else{
            self.obj.some.$dials.addClass("hide");   
        }
    };
    ...
    this.saveSomeTabsSubmit = function(){
        self.obj.some.$title.removeClass("error");
        self.obj.some.$list.find(".in-edit").find(".item-edit").trigger("click");
        ...
        if(!inEdit.length){
            ...
            var title  = String(self.obj.some.$title.val() || "");
            
            if(!title || !title.replace(/^\s+/g, '').length){
                setTimeout(()=>{
                    self.obj.some.$title.addClass("error");
                }, 100);
    ...
    this.logoutButtonPrepare = function(){
        if(head.browser.name == 'ff' && false){// merge
    ...
    this.logout = function(){
        ...
        localStorage.setItem("auth-need-logout", 1);
        AUTH.logout(true);
        self.setPanelSize();
    };
