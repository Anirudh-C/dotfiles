...
function protectElements(){
    var $elements = $("#mv-tiles, #mv-clock, #sidebar-wrap, #todo-container, #header, #footer, #relax, #header-weather, .bba-wrap, #focus-newtab, #footer-main, #searchChoiseButton");
    ...
    if(locked && locked == "on"){
        ...
        if(typeof relaxSwitcher == "function") relaxSwitcher("stop");
    }else{
...
function PassCode(data){
    ...
    self.API = {
        server: "https://livestartpage.com/pinmail.php"
    };
    ...
    this.unlock = function(form, hash){
        if(self.isLock()){
            ...
            console.info('Current', crypt);
            console.info('Restore', self.data.restore);
            console.info('Pass', self.data.pass);
                    ...
                    function(){
                        ...
                        setTimeout(function(){
                            ...
                            if(!cleared){
                                ...
                                self.reloadOtherAddonPages("settings");// Task #757
                            }//else
                        }, 150);
                    },
                    (cleared ? false : true)
...
//              if(form == "settings"){
    ...
    this.reloadOtherAddonPages = function(filter){
        if(filter && typeof filter != "object") filter = [filter];
            
        setTimeout(function(){
            BRW_TabsGetCurrentID(function(tab) {
                if(!filter || filter.indexOf('new') !== -1) getNetTabPages(reloadTabPages, {skipTab: tab});
                if(!filter || filter.indexOf('gallery') !== -1) getOptionsTabPages(reloadTabPages, {skipTab: tab});
                if(!filter || filter.indexOf('settings') !== -1) getSettingsTabPages(reloadTabPages, {skipTab: tab});
                if(!filter || filter.indexOf('favorite') !== -1) getFavoriteTabPages(reloadTabPages, {skipTab: tab});
            });
        }, 150);
    },
