    ...
    setTimeout(function(){//Wait for loading
        if(isAuthEnable()){
            ...
            if(_PAGE == "newtab"){
                ...
                SYNC.autoUpdate(12*60*60*1e3);
            }
                        
            SYNC.listenUpdates(5*60e3);
        }
    }, 650);
...
function authButtonInfo(state, message){
    var $link = $("#sync-link");
    var $img  = $("#sync-link").find(".footer-link-ico img");
    
    if(message) $img.attr("title", message);
    else $img.attr("title", translate("sync_button_footer"));
...
function SyncLSP(data){
    ...
    self.state={
        ...
        ignoreClick : false,
        pickUpInterval: false,
        noSilent : false,
        syncDials: getDisplaySpeedDialPanel() ? true : false,
        autoUpdateInterval: 12*60*60*1e3,
    };
    ...
    this.init = function(){
        ...
        $button.on("click", function(){
            ...
            self.state.noSilent = true;
            
            //console.info(self.state.inProgress, lastProgress, (Date.now() - lastProgress > 60000));
    ...
    this.syncTodoCheck = function (run, callBack) {
        ...
        self.syncFavorites({},
            function (fActions) {
                self.startSyncToDo(run);
            
            },
            function (fError) {
                console.warn("Sync Favorites ERR", error);
                self.startSyncToDo(run);
            }
        );
    },
        
    this.startSyncToDo = function(run){
        self.syncToDo({},
            function (actions) {
                if(self.state.refresh.todo == true){//Update ToDo UI if necessary
                    if(typeof TODO == "object" && typeof TODO.refresh == "function") TODO.refresh('lsp');
                    else BRW_sendMessage({"command": "todoListUpdate"});
                }

                if(self.state.syncDials){
            ...
            function (error) {
                console.warn("Sync TodoCheck ERR", error);
            }
        );
    },
                ...
                self.syncGroupsBoth({Found:Server}, function(actions){
                    ...
                    self.syncDialsBoth(actions, function(){
                        if(self.state.refresh.dial){
                            ...
                            else BRW_sendMessage({"command": "sidebarDialsUpdate"});
                        }//if
                                ...
                                BRW_dbSelect({
                                        ...
                                        function (Client) { //Success
                                                    ...
                                                    case "server add": case "server update":
                                                        ...
                                                        if(
                                                            ...
                                                            (
                                                                ...
                                                                ||
                                                                ClientByID[idClient].image.indexOf('blob:moz-extension') === 0// merge
                                                                //|| // for Firefox
                                                            )
                                                        ){ // Local image file
                                            ...
                                            if(sendServerAdd.dials.length){//Send "add" to server
                                                //console.info("Dials to srv", sendServerAdd);
                                                ...
                                                self.ajax(sendServerAdd, function(addResp){
                                                    ...
                                                    if(
                                                        typeof addResp == "object" 
                                                        && typeof addResp.extra == "object" 
                                                        && typeof addResp.extra.notFoundGroups == "object" 
                                                        && addResp.extra.notFoundGroups.length 
                                                    ){
                                                        self.putFullGroupsList();
                                                    }
                                                }, function(addErr){
    ...
    this.putFullGroupsList =()=> {
        console.info('putFullGroupsList');
        ...
        BRW_dbTransaction(function(tx) {
            BRW_dbSelect(
                ...
                function(Client){
                    ...
                    var ClientIDs = [], ClientByID = {}, groups = [];

                    for(var key in Client.rows) if((Client.rows[key].id) && (Client.rows[key].id != 10000000000000))
                    {
                        groups.push({
                            global_id: Client.rows[key].type == 0 ? 'default' : Client.rows[key].id,
                            name: Client.rows[key].title,
                            position: Client.rows[key].orderNum,
                        });
                    }
                    ...
                    self.ajax(sendServerFull, function(addResp){
                        console.log("putFullGroupsList", sendServerFull, addResp);
                        self.setUpTime(["groups"], addResp.lastUpdateTime);
                    }, function(addErr){
                        console.warn(addErr);
                    });
    ...
    this.filesPreUpload =(idClient, ClientItem, ServerItem)=> {
        ...
        if(ClientItem.image.indexOf("filesystem:") === 0){
            ...
            xmlHTTP.onerror = function(err) {console.warn("Image ERR", err);};
            xmlHTTP.send();
        }
        else
        if(ClientItem.image.indexOf("blob:resource") === 0 || ClientItem.image.indexOf("blob:moz-extension") === 0){// merge
                    ...
                    var tryUpdateDialImageTimer = setInterval(function(){
                        if(self.state.synchronized){
                            ...
                            self.ajax(
                                ...
                                function (addErr) {
                                    console.warn("Can't push dial", addErr);
                                }
                            );
                        }
                    }, 1500);
                ...
                self.ajax(
                    ...
                    function(ServerUPD, ErrorUPD){//Success
                        if(
                            ...
                            var Client = getFavoriteThemesObject('gallery') || {};
                            ...
                            for(var key in Server.items){                                
                                ...
                                if(!val.value.downloadedByUser){
                                    //console.info(val.value);
                                    sDateUUID[uuid] = val.uuid;

                                    sUUID.push(uuid);
                                    sValUUID[uuid] = val.value;
                            ...
                            if(serverDelete.length){
                                self.ajax(
                                    ...
                                    ,
                                    function(Err){//Error
                                        console.warn("Favorites DEL ERR", Err);
                                    }//Error
                                );
                            }
                                ...
                                for(var kBoth in BothUUID){
                                    ...
                                    if(
                                        ...
                                        if(
                                            ...
                                            ||
                                            (CbyUUID[vBoth].due != SbyUUID[vBoth].due)
                                        ){//Param
                                            if(
    ...
    this.todoUUID = function(item){
        ...
        var date = new Date(parseInt(item.id) || item.id);
    ...
    this.autoUpdate = function(interval){
        ...
        self.state.pickUpInterval = setInterval(function(){
            ...
            if(
                ...
                &&
                (now - last) >= self.state.autoUpdateInterval
            ){
                self.autoStart();
            }else{
                self.pickUpSync();
    ...
    this.autoStart = function(){
        AUTH.check(false, function(){
            ...
            console.info('Auto Sync', new Date());

            self.syncStart();
        });
    },
        
    this.errorMessage = function(type, mode){
        if(mode == "silent" && self.state.noSilent){
            self.state.noSilent = false;
            mode = false;
        }
        
        switch(type){
            case "network":
                authButtonInfo("error", translate("error_sync_network"));
                
                if(mode != "silent"){
                    $.jGrowl(translate("error_sync_network"), { "life" : 3000, position: "top-left"});
                }
                
                self.syncProgress("break");
            break;
        }
    },
        ...
        if(status == "on"){//on
            ...
            if(!self.state.syncTimer){
                self.state.syncTimer = setInterval(function(){
                    if(self.state.inProgress) self.syncTimer("on");
                }, 1000);
            }
        }else{
