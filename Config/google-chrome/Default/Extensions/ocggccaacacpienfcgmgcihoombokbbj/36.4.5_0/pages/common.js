 ...
 */
    var LSP = {
        env: (typeof document !== 'object' || String(document.title).indexOf('Backend') !== -1) ? 'bg' : 'front'
    };
    ...
    var themesApiCacheTime = 24*60*60e3; // 1 day // 15 min
    ...
    var searchProviders = {
        ...
        "searchProviderBaidu" : {'name':'Baidu', 'val':searchProviderBaidu},
    };
    ...
    //var flixeFileSource = "https://flixels.s3.amazonaws.com/flixel/";// Task #1546
    var flixeFileSource = "https://d364tc25z85lp9.cloudfront.net/flixel/";// Task #1546
    ...
    if(h264 === null){
        h264 = html5VideoDetectH264();
        localStorage.setItem("html5-video-h264", h264);
    }
    ...
    var downloadedLiveBackgroundContent = {
        ...
        */
        ...
        "fullHd" : 1,
        "handmade" : 0,
        "bgPoster" : getFlixelFileThumb("8s1nok6ep6r1f65i34p2"), //"/default-content/video/8s1nok6ep6r1f65i34p2.thumbnail.jpg", //getFlixelHdVideo("8s1nok6ep6r1f65i34p2"),
    };
    ...
    //var accuweatherServer = "http://api.accuweather.com"; // Task #1677
    var accuweatherServer = "http://dataservice.accuweather.com";   
    //var accuweatherKeyDefault = "ca8f6bdce94249069b6ce81976e05f6d";//demo
    var accuweatherKeyDefault = "KGuF7cU3n2jl78OpUj1wBXhzDJSaJ6Lu";//prod
    ...
    var defaultWeatherBackgroundOpacity = 0.595;//4;
    var minWeatherBackgroundOpacity = 0.1;
    var maxWeatherBackgroundOpacity = 1;

    var defaultTodoPanelOpacity = 0.76;
    var defaultBottomPanelOpacity = 0.7;
    var minBottomPanelOpacity = 0.20;
    ...
    var termsDisallowLocations = ["RU", "UK", "UA", "UZ", "KG", "KZ", "BY", "AR", "AT", "BR", "CA", "CH", "CL", "CO", "DE", "DK", "ES", "FI", "FR", "HK", "IN", "ID", "IT", "MX", "NZ", "NO", "PE", "PH", "SG", "SE", "TW", "TH", "GB", "UK", "US", "IE", "VE", "MY"];

    var debugLevelsArray = localStorage.getItem('debug-level') ? localStorage.getItem('debug-level').split(',') : [];

    function debugLevel(level){
        if(level === false || level === 0 || level === "off" || level === "clear"){
            debugLevelsArray = [];
        }else if(level){
            var level = String(level);
            var index = debugLevelsArray.indexOf(level);
            
            if(level[0] !== '-'){
                if(index === -1) {
                    debugLevelsArray.push(level);
                }
            }else{
                if(index !== -1) {
                    debugLevelsArray.splice(index, 1);
                }
            }
        }
        
        localStorage.setItem('debug-level', debugLevelsArray.join(','));
        
        return debugLevelsArray;
    }

    function Log(data, mode){
        if(typeof mode != "object") mode = {level: mode || 'common'};
        if(!mode.level) mode.level = 'common';
        
        try{
            if(debugLevelsArray.indexOf(mode.level) !== -1){
            ...
            if(debugLevelsArray.indexOf("self") !== -1){
                console.warn(ex);
            }
    ...
    function getRedirectSearchUrl(query, engine) {
        ...
        if(!engine || engine == searchProviderLive || engine == searchProviderYandex || engine == searchProviderBing) { // Task #2040
            //console.info("engine", engine, "searchProviderLive", searchProviderLive, "searchProviderYandex", searchProviderYandex);
            ...
            }else if(engine == searchProviderBaidu){
                var url = "https://www.baidu.com/s?wd="+query;   
            }
        }
    ...
    function getVideoThumb(theme){
        if(!theme.handmade) return getFlixelVideoThumb(theme.id);
        else return getThemeVideoThumb(theme.id);
    }
    ...
    function getBottomPanelOpacity() {
        var opacity = parseFloat(localStorage.getItem("page-bottom-panel-opacity")) || 0;
    ...
    function getSearchFormProviderType(callback, scrict) {
        acceptedLocales((languages)=>{// Task #1662
            var hasRuLanguage = languages.lang.indexOf("ru") !== -1;
    ...
    function setSearchFormProviderType(val, callback) {
        ...
        localStorage.setItem('current-search-provider', String(val));
    }
        ...
        if(!isNaN(id) && !isNaN(order) && id && order) {
            BRW_dbTransaction(function (tx) {
                ...
                BRW_dbInsert(
                    ...
                    function(){//success
                        BRW_sendMessage({command: "tasksUpdated"});
                    },
                    function(){//error
     ...
     * @param title String
     */
    function changeTodoItemCommon(item){
        var id = parseInt(item.id);
        
        if(!id){
            console.warn('Has no ID', item);
            return;
        }
        
        let setData = {};
        for(var k in item) if(k !== 'id') setData[k] = item[k];
        
        if(!Object.keys(setData)){
            console.warn('Data is empty', item);
            return;
        }
        ...
        BRW_dbTransaction(function (tx) {
            BRW_dbUpdate(
                ...
                function(ex){//error
                    console.warn('ERR changeTodoItemCommon', item , setData, ex);
                }
            );
        });
    }
    ...
    function getDisplayTodoSize() {
        ...
        return val ? JSON.parse(val) : {"width" : "414px", "height" : "340px"};
    }
...
/**
    ...
    var reloadAllTimeout;
    function reloadAllPages(timeout, except){
        timeout = timeout || 0;

        if(typeof except != 'object') except = [except || 'none'];

        clearTimeout(reloadAllTimeout);


        reloadAllTimeout = setTimeout(function(){
            getNetTabPages((tabs)=>{
                getSettingsTabPages((tabsS)=>{
                    getOptionsTabPages((tabsO)=>{
                        for(var i in tabsS.tabs) tabs.tabs.push(tabsS.tabs[i]);
                        for(var i in tabsO.tabs) tabs.tabs.push(tabsO.tabs[i]);
    ...
    function reloadTabPages(data) {
        ...
        if(dont){
            if(dont == "once" || dont.indexOf('soft:') !== -1){
                localStorage.removeItem("dont-reload-tabs");
                reload = false;
                ...
                if(dont[1]){
                    if(dont[0] == "soft"){
                        BRW_sendMessage({command:dont[1]});
                    }
                }
            }else{
    ...
    function addThemeInstalledElement(theme, fileName, fileResolution, fileType, contentType) {
        ...
        if(typeof (themes[theme.id]) == "undefined") {
            ...
            var handmade = typeof theme.handmade == "undefined" || theme.handmade ? 1 : 0; // Task #851 
     ...
     * @return Object
     */
    function getFavoriteThemesObject(mode) {
        if(typeof mode !== 'string') mode = false;
        var result = {};
        ...
        if(favoriteThemesData) {
            ...
            for(i in favoriteThemesList) {
                ...
                if(mode == "gallery" && favoriteTheme.downloadedByUser == 1)  continue;
...
/**
    ...
    function openUrlInNewWindow(url){
         BRW_openUrlInNewWindow(redirectModification(url));
    }
    ...
    function getAlternativePlaceFinder(search) {
        ...
        //return "https://query.yahooapis.com/v1/public/yql?format=json&q=" + encodeURIComponent('SELECT * FROM geo.places WHERE text="'+search+'" AND placeTypeName.code = 7');
    }
    ...
    function getAlternativePlaceWeather(place, callback, isKey) {
        ...
        $.getJSON(placeUrl, function(e) {
            ...
            if(placeInfo && placeInfo.woeid) {
                ...
                try{
                    placeLocation.area = (placeInfo.admin1.content != placeInfo.locality1.content) ? placeInfo.admin1.content : placeInfo.placeTypeName.content;
                }catch(ex){
    ...
    function getAccuweatherKey(){
        //return localStorage.getItem("weather-hash") || accuweatherKeyDefault;// Task #1677
        return accuweatherKeyDefault;
    }
            ...
            function(response){//successFunction
                ...
                if (response && response[0]) {
                    ...
                    var location = {
                        ...
                        "area": (found.AdministrativeArea.LocalizedName != found.LocalizedName ? found.AdministrativeArea.LocalizedName : found.AdministrativeArea.LocalizedType),
    ...
    function weatherErrorCantLoad(){
        $(".circleGM").remove();
        jGrowlApply(translate("weather_cant_load"), { "life" : 3000, position: "top-right"});
    }
    ...
    function weatherShowErrorMessage(name){
        ...
        if(name){
            jGrowlApply(translate("weather_not_found") + ": <b>" + name, { "life" : 3000, position: "top-right"});
        }
    }
     ...
     * @param callback Function
     */
    function getPlaceWeatherAccuweather(geo, callback, noWeather) {
            ...
            function(response){//successFunction
                if(typeof response == "object" && response.Key){
                    var location = {
                        ...
                        "area": (response.AdministrativeArea.LocalizedName != response.LocalizedName ? response.AdministrativeArea.LocalizedName : response.AdministrativeArea.LocalizedType),
                        //add more information
                    };
    ...
    function getWeatherAccuweather(id, placeLocation, callback) {
        if (id) {
            getCountryTemperatureUnit(placeLocation.countrycode, function (unit) {
                //var url10Day = getAccuweather10DayURL(id, unit); //Task #1677
                var url5Day = getAccuweather5DayURL(id, unit);
                //var url24Hour = getAccuweather24HourURL(id, unit); //Task #1677
                var url12Hour = getAccuweather12HourURL(id, unit);
                
                var xhr5Day = BRW_ajax(
                    url5Day,
                    function (data5Day) { //successFunction
                        //console.log("5 Days", data5Day);

                        var xhr12Hour = BRW_ajax(
                            url12Hour,
                            function (data12Hour) { //successFunction
                                var weather = {
                                    code: data12Hour[0].WeatherIcon,
                                    date: data12Hour[0].DateTime,
                                    temp: data12Hour[0].Temperature.Value,
                                    text: data12Hour[0].IconPhrase,
                                    precipitation: data12Hour[0].PrecipitationProbability
                                }

                                var locationWeather = {
                                    "location"   : placeLocation,
                                    "weather"    : weather,
                                    "unit"       : data12Hour[0].Temperature.Unit.toLowerCase(),
                                ...
                                setWeatherForecast({"Hourly":data12Hour, "Daily":data5Day});
                                callback(locationWeather);
                            },
                            function (error12Hour) { //errorFunction
                                accuweatherUnavailable();
                                console.warn("Can not load 12 hours weather forecast", error24Hour);
                            }
                        );
                    },
                    function (error5Day) { //errorFunction
                        accuweatherUnavailable();
                        console.warn("Can not load 5 days weather forecast", error5Day);
                    }
                );
            });
        }
            ...
            getCountryTemperatureUnit(placeLocation.countrycode, function(unit) {
                ...
                $.getJSON(url,function(e) {
                    ...
                    if(e && e.query && e.query.results && e.query.results.channel) {
                        ...
                        setLastLocationWeather(locationWeather);
                        setWeatherForecast(weatherChanel);
                        callback(locationWeather);
                    }
                });
            });
     ...
     * @returns {boolean}
     */
    function checkUrlHasGoogleHost(url, checkLocale) {
        var result = false;
        
        if(BROWSER && BROWSER == 'firefox') return false;//firefox
        if(checkLocale && !isAcceptedLocSeries()) return false; // Task #865
        ...
        BRW_getUILanguage(function(lang){
            ...
            if(lang){
                ...
                if(lang){
                    ...
                    $container.find("[setlocaleimg]").each(function(){
                        var Locales = String($(this).attr("setlocaleimg")).split(',');
        ...
        switch(String($element[0].tagName).toLocaleLowerCase()){
            ...
            default:
                if($element.attr("data-toggle") == "tooltip"){
                    if(text.indexOf("<a") != -1 || $element.hasClass('auth-feauture-help')){
                    ...
                    $element.attr("title", text);
                }else if($element.attr("lang-place") == "title"){
                    $element.attr("title", text);
                }else{
...
function dialogConfirm(data){
    ...
    if(data.message){
        ...
        else if(typeof data.message == "object") $modal.find(".dialog-message").append(data.message);
    }
...
function addonNotification(message) {
    browser.tabs.query({active: true}, function(Tabs) {// Task #1184
        var Tab = Tabs[0];
...
var cachedImages=[];
function noCacheImage(url, force){
    var url = String(url);
    ...
    if(
        ...
        if(cachedImages.indexOf(url) === -1) cachedImages.push(url);
        
        if(cachedImages.indexOf(url) !== -1 || force){
...
function secToTime(timeSec){
    var min = String("0"+Math.floor(timeSec/60)).substr(-2);
...
function getLocationByService(cb){
    ...
    var last = String(localStorage.getItem("service-location-session") || "never");
    var session = String(localStorage.getItem("addon-session") || "null");
    
    if(current && last === session) cb(current);
    else{
        Log(["getLocationByService", 'AJAX'], last, session);
        ...
        $.ajax({
            ...
            success : function(data) {
                ...
                localStorage.setItem("service-location-session", session);
                
                cb(location);
            },
            error : function(error){
                //console.warn(error);
                if(localStorage.getItem("service-location")) cb(localStorage.getItem("service-location"));
                else
...
function isAcceptedLocation(cb){    
    try{
        getDisplayWeatherPanel(function(display) {
            if(display) {
                getLastLocationWeather(function(weather){
                    if(
                        weather != null &&
                        ...
                        Log(['isAcceptedLocation', 'weather', weather.location.countrycode], 'logo');

                        localStorage.setItem("weather-location", weather.location.countrycode);
                        isAcceptedCheck(weather.location.countrycode, cb);
                    }else{
                        localStorage.removeItem("weather-location");
                        isAcceptedLocationByService(cb);
                    }
                });
            }else{
                localStorage.removeItem("weather-location");
...
function isAcceptedCheck(loc, cb){
    var accepted = true;
    ...
    Log(['isAcceptedCheck', loc, accepted], 'logo');
...
function isAcceptedLocationByService(cb){
    getLocationByService(function(loc){
        Log(['isAcceptedLocationByService', loc], 'logo');
        
        isAcceptedCheck(loc, cb);
    });
}

function isAcceptedLocSeries(){
    var loc = localStorage.getItem("weather-location") || localStorage.getItem("service-location") || localStorage.getItem("definedLocation");
    
    var result = false;
    
    if(loc) result =  isAcceptedCheck(loc);
    
    Log(['isAcceptedLocSeries', loc, result], 'logo');
    
    return result;
}

function commonAddFlixelHdVideoIfNotSetForFullHd(contentItem) {
...
var jGrowlLastMsg = "", jGrowlTimeout;
function jGrowlApply(message, param){
    if(
        message != jGrowlLastMsg 
        || 
        (typeof param == "object" && param.force)
    ){
        jGrowlLastMsg = message;
        
        clearTimeout(jGrowlTimeout);
        
        $.jGrowl(message, param || {});

        jGrowlTimeout = setTimeout(()=>{
            jGrowlLastMsg = "";
        }, 5000);
    }
}

function acceptedLocales(callback, info){
    BRW_getAcceptLanguages(function(languagesFull){
        var languages = [], langAdv = [];
        
        for(var i in languagesFull){// Task #1662
            var lang = languagesFull[i];
            
            if(lang.indexOf('-') ===  -1){
                if(languages.indexOf(lang) === -1) languages.push(lang);
            }else{
                lang = lang.split('-').shift();
                if(langAdv.indexOf(lang) === -1) langAdv.push(lang);
            }
        ...
        if(info){
            console.info(languagesFull, languages);
        }

        if(typeof callback == "function"){
            callback({ // Task #1286 (UK)
                lang: languages,
                RU  : languages.indexOf("ru") != -1,
                UK  : languages.indexOf("uk") != -1,
                CIS :
                    languages.indexOf("ru") != -1 
                    || languages.indexOf("by") != -1 || languages.indexOf("be") != -1 
                    || languages.indexOf("ua") != -1 || languages.indexOf("uk") != -1 
                    || languages.indexOf("kz") != -1 || languages.indexOf("kk") != -1 
                    || languages.indexOf("kg") != -1 
                    || languages.indexOf("uz") != -1 
                    || languages.indexOf("ge") != -1 || languages.indexOf("ka") != -1 
                    || languages.indexOf("az") != -1 
                    || languages.indexOf("md") != -1 || languages.indexOf("mo") != -1
            });
        }
    });    
}

var getVideoPosterResult = false;
function getVideoPoster(){// Task  #1587
    if(getRandomThemesDisplay()){
        BRW_getFileSystem(function(){
            BRW_sendMessage({command: "getBackgroundImage"}, function(response) {
                getVideoPosterResult = response;

                var themeId = String(response.video || response.image).split('/');
                themeId.pop();
                themeId = themeId.pop();

                loadVideoPoster(themeId);
            });
        });
    }else{
        loadVideoPoster();
...
function loadVideoPoster(randomizeThemeId){
    if(randomizeThemeId) var themeId = randomizeThemeId;
    else var themeId = localStorage.getItem("background-video-file");
    
    if(!themeId){
        if(typeof applyVideoPoster == "function") applyVideoPoster(null);
    }else{
        themeId = themeId.split('/').shift();
        
        var posterId  = localStorage.getItem("background-video-poster-id") || false;
        var posterErr  = parseInt(localStorage.getItem("background-video-poster-err")) || 0;
        var posterErrId = localStorage.getItem("background-video-poster-err-id") || false;
        var posterImage = localStorage.getItem("background-video-poster-image") || false;
        
        if(posterErrId != themeId){
            posterErr = 0;
            localStorage.removeItem("background-video-poster-err");
        }
        
        if(posterImage && posterId == themeId){
            //console.timeEnd("getVideoPoster"); console.groupEnd("getVideoPoster"); 
            if(typeof applyVideoPoster == "function") applyVideoPoster(posterImage);
        }else
        if(posterErr >= 3){
            applyVideoPoster(null);
        }else{
            var installed = JSON.parse(localStorage.getItem("installed-themes"));
            
            var theme = installed[themeId];
            
            if(typeof theme != "object" || !theme.bgPoster){
                if(typeof applyVideoPoster == "function") applyVideoPoster(null);
            }else{
                if(theme.bgPoster.indexOf('flixel') !== -1 && theme.bgPoster.indexOf('hd.mp4') !== -1){//fix
                    theme.bgPoster = getFlixelFileThumb(themeId);
                }else
                if(theme.bgPoster.indexOf('http') === -1 && theme.bgPoster.indexOf('default-content') === -1){//fix
                    if(theme.isFlixelContent) theme.bgPoster = getFlixelFileThumb(themeId);
                    else theme.bgPoster = getThemeFileThumb(themeId);
                }
                
                loadPosterImage(theme.bgPoster, theme,
                    (response)=>{
                        localStorage.setItem("background-video-poster-id", themeId);
                        localStorage.setItem("background-video-poster-image", response);
                        localStorage.removeItem("background-video-poster-err");
                        
                        if(typeof applyVideoPoster == "function") applyVideoPoster(response);
                    },         
                    (error)=>{
                        localStorage.setItem("background-video-poster-err", ++posterErr);
                        localStorage.setItem("background-video-poster-err-id", themeId);
...
function loadPosterImage(url, theme, onSuccess, onError) {
        ...
        if(xhrBgImage.readyState == 4) {
            ...
            if (this.status == 200) {
                ...
                getFileSystem(saveFile, {
                    ...
                    "download" : true,
                    "callback": (response)=>{
                        //console.timeEnd("loadPosterImage"); console.groupEnd("loadPosterImage"); 
                        onSuccess(response);
                    }
                });
            } else {
                console.warn('Error by path', url);
    ...
    xhrBgImage.onerror = function (errorMessage) {
        console.warn('Error: ' + errorMessage + ' by path', url);
        if(typeof onError == 'function') onError();
    };
...
function merge(obj1,obj2){
    ...
    if(typeof obj1 === "string" && obj1[0] == '{' && obj1.slice(-1) == '}'){
        try{
    ...
    if(typeof obj2 === "string" && obj2[0] == '{' && obj2.slice(-1) == '}'){
        try{
...
function checkIndexedDB (callback) {// merge
    var state = {incognito:false, storageFail:false};
...
function resolveBlobUrl(mv, callback){// merge
    var image = mv.image;
...
function getAddonData(){
    var data = [
        "type=lsp_" + browserName(true),
        "version=" + localStorage.getItem("addon-version"),
        "installed=" + localStorage.getItem("install-key") || localStorage.getItem("installed-key"),
    ];
...
function urlify(text) {
    var urlRegex = /((((https?|ftp):)?\/\/|www\.)[^\s]+)/g;
...
function fallbackCopyTextToClipboard(text, cb) {
    ...
    try {
        ...
        if(successful) {if (typeof cb == "function") cb();}
        else console.warn('Fallback: Copying text command was ' + msg);
    }
    catch (err) {
...
function copyTextToClipboard(text, cb) {
    if (!navigator.clipboard || BROWSER == "firefox") {
...
function numEnding(iNumber, word, aEndings) {
    // aEndings for [1, 4, 5]
    
    var sEnding = '', i, loc = "en";
    //var loc = String(localStorage.getItem("definedLocation") || localStorage.getItem("service-location")).split('-').shift().toLowerCase();
        
    iNumber = parseInt(iNumber) || 0;
    
    if(!aEndings) aEndings = translate('endings').split('|');
    if(typeof aEndings == "string") aEndings = aEndings.split('|');
    
    if(aEndings.length == 3) loc = "ru";
    
    if(loc !== "ru"){
        if(iNumber > 1) sEnding = aEndings[1];
        else sEnding = aEndings[0];
    }else{//ru
        iNumber = iNumber % 100;
...
function cutSeconds(time){
    return time.replace(/(\d\d?)(\:|\.)(\d\d?)(\:|\.)(\d\d)/, '$1$2$3');
}
...
function getTypeOf(variable, compare) {
    var result;
    
    if(typeof variable == "object"){
        if(typeof variable.then == "function" && typeof variable.catch == "function"){
            result = "promise";
        }else
    ...
    if(compare) return result == compare;
    else return result;
}

function genId(){
    return String(Date.now()+'_'+Math.round(1e4*Math.random()));
}