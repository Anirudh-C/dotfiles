                ...
                if(group) {
                    ...
                    $tilesContainer.sortable({
                        ...
                        start: function (ev, ui) {
                            tilesInMove = true;
                            dragDial($(ui.item), "start");
                        },
                        stop: function (ev, ui) {
                            dragDial($(ui.item), "stop");
    ...
    var dialMoveGroup = false;
    function dragDial($dial, mode){ // Task #1214
        var $groups = $('#sidebar-wrap .sidebar-group-item:not(.selected):not(.group-is-popular)');
        
        if(mode == "start"){
            $("body").attr("dragdial", "1");
            
            $dial.addClass("mv-dial-sortable");
            
            //var winWidth = document.body.offsetWidth;
            
            //$groups.append($('<span>').addClass('groupCurtain'));
            
            dialMoveGroup = false;
            var coords = {}, wArea = false;
            ...
            $groups.each((N, el)=>{
                ...
                if(!wArea) wArea = {left:offset.left, right:offset.right};
                
                coords[$(el).attr('data-group')] = offset;
            });
            
            //console.info(wArea, coords);
            
            $("body").unbind("mousemove").on("mousemove", function(event){
                var X = event.pageX, Y = event.pageY;
                var group = false;
                
                if(X >= wArea.left && X <= wArea.right){
                    //console.info(event.pageX, event.pageY);
                    
                    for(var id in coords){
                        if(Y >= coords[id].top && Y <= coords[id].bottom){
                            group = id;
                            break;
                        }
                    }
                ...
                if(group != dialMoveGroup){
                    dialMoveGroup = group;
                    
                    //console.info(dialMoveGroup);
                    
                    $groups.removeClass('move-to-group');
                    
                    if(dialMoveGroup){
                        $groups.filter("[data-group="+dialMoveGroup+"]").addClass('move-to-group');
                    }
                }
            });
            
        }else{
            $("body").unbind("mousemove").removeAttr("dragdial", "1");
            
            $dial.removeClass("mv-dial-sortable");
            ...
            if(!dialMoveGroup){
                ...
                var editedDial = {
                    "id" : $dial.attr("data-dialid"),
                    "groupId" : dialMoveGroup,//$dial.attr("data-groupid"),
                    "url" : $dial.attr("fullhref"),
                    "title" : $dial.attr("title"),
                    "addDate": parseInt($dial.attr("add-date")) || 0,
                };
                
                $dial.css("display","none");
                
                //console.info(dialMoveGroup, editedDial);
                
                confirmMoveDialToNewGroup(editedDial, true, false, true);
            }
        }
    }
...
*/
    ...
    function displayAddNewDial() {
        ...
        try{
            if(!VAL.get('show-plus-dial')) $dial.addClass("hide");
        }catch(ex){
            console.warn(ex);
        }
        ...
        $dial.on("click", (e)=>{showAddNewDialPopup(e)});
...
*/
    ...
    function clickDial($el, whichTab){
        ...
        if(!tilesInMove){
            var url = String($el.attr("fullHref"));
            
            console.info(url);
            
            if(url.indexOf('file:///') === 0 && browserName(true) == "ff"){
                dialogConfirm({
                    title   : translate("local_file_dial_title"), 
                    message : translate("local_file_dial_message"), 
                    confirmTxt: translate("close"), 
                    cancelHide: true
                });
            }else{
                BRW_sendMessage({
                    command: "openSelectedDialUrl", url: url,
                    newtab: whichTab
                });
            }
    ...
    function displayTileItem(mv, i) {     
        ...
        var $tile = $("<a></a>");
            ...
            if(mv.addDate) $tile.attr("add-date", mv.addDate);
            else $tile.attr("add-date", Date.now());
        ...
        var title = String(mv.title).replace('http://', '').replace('https://', '').replace('ftp://', '').replace('file:///', '');
        if(String(title).indexOf('&') !== -1) title = decodeEntities(title);
        ...
        if((mv.image && mv.image == getNoTileImageFileName()) || mv.thumbType == showDialTextThumb) {
            ...
            if(checkUrlHasGoogleHost(mv.url, true)  && (!mv.thumbType || mv.thumbType == showDialGalleryThumb) && mv.image.indexOf("data:image") == -1 && mv.image.indexOf("filesystem") == -1 && mv.image.indexOf("resource") == -1) {
                if(typeof (mv.colorScheme) == "undefined") {
                    $thumb.append($dotBg.append($dotImage));
                    needLoadTextTilesList.push(mv);
                } else{
                ...
                if(mv.image.indexOf("data:image") < 0/* && mv.image.indexOf("blob:") < 0*/) {
                    $thumbImg = $("<div></div>");
                    ...
                    resolveBlobUrl(mv, (imageUrl)=>{// merge
                        $thumbImg.css({"background" : "url('" + imageUrl + "') 50% 50% / contain no-repeat"});
                    });
    ...
    function displayTileItemReDraw($thumbImg, mv){//firefox
        //console.info('displayTileItemReDraw', $thumbImg, mv);
        
        if(BROWSER && BROWSER == 'firefox' && mv.image && mv.image.indexOf("blob:") === 0){
			// merge
            if(isAmazonProductURL(mv.url)) var Name = mv.hostUrl + "_" + crc32(mv.url);// + ".jpg";
            else var Name = mv.hostUrl;
    ...
    function displaySearch(repeat) { 
        ...
        getDisplaySearchForm(function(display) {
            waitAuth(function(){
                if(!display && !repeat && !AUTH.isPremium()){ // Task #2026
                    setDisplaySearchForm(1);
    ...
    function searchEngineSelector(){
        ...
        getSearchFormProviderType(function(searchPorviderType) {
            acceptedLocales((Accepted)=>{
                var waitAuthN = 0, waitAuth = setInterval(()=>{
                    //console.debug(searchPorviderType, Accepted);
                    ...
                    if(typeof AUTH == "object" || ++waitAuthN > 10){
                        ...
                        for(var key in searchProviders){
                            if(key == "searchProviderYahoo") continue; //Task #1661
                            ...
                            if(
                                (!premium) // Task #2047
                                &&
                                (key == "searchProviderGoogle" || key == "searchProviderDuckDuckGo" || key == "searchProviderBaidu")
                            ){ // Task #2026
                                $option.addClass('search-provider-pro').addClass('hidden');
                            }

                            if(key == "searchProviderYandex" && !Accepted.CIS){ // Task #2026
                                $option.addClass('hidden');
                            }

                            /* // Task #2041
                            if(
                                !premium  &&  Accepted.CIS && !Accepted.UK 
                                && (searchProviders[key].val !== 1 ) && (searchProviders[key].val !== 7 ) 
                                && (searchProviders[key].val !== 2 || !Accepted.RU)
                            ){
    ...
    function endDisplayLoadedTileThumbImage($el, mv) { 
        if(mv.id){// merge
            ...
            if(tileThumbImageCache[mv.id] && tileThumbImageCache[mv.id] == UPD){
                return;
            }
        ...
        setTimeout(function () {
            ...
            if ((mv.image && mv.image == getNoTileImageFileName()) || mv.thumbType == showDialTextThumb) { // display no tile image
                ...
                if (checkUrlHasGoogleHost(mv.url, true) && (!mv.thumbType || mv.thumbType == showDialGalleryThumb) && mv.image.indexOf("data:image") == -1 && mv.image.indexOf("filesystem") == -1 && mv.image.indexOf("resource") == -1) {
                    ...
                    if(mv.image.indexOf("data:image") === -1) {
                        $mvThumb.addClass("mv-thumb-image");
                        
                        $thumbnail.css({
                            "background": "url('" + noCacheImage(mv.image, true) + "') 50% 50% / contain no-repeat"
                        });
                        ...
                        if (mv.thumbType == showDialGalleryThumb) {
                            ...
                            $thumbnail.css({"background": "url('" + noCacheImage( mv.image, true )  + "') 50% 50% / contain no-repeat"});
                            $thumbnail.attr("image", mv.image);
    ...
    function dialsPanelVisibleChange(e, mode) {
        ...
        if( $targetEl[0].tagName.toLowerCase() != "html"
            &&
            ["search-container", "footer-visible-dials", "dials-notification", "dials-notification-message", "mv-container", "mv-clock-wrap", "mCSB_3_container", "mCSB_4_container", "mv-tiles-wrap-wide", "mv-tiles-wrap", "mv-tiles", "mCSB_3", "mCSB_4", "relax", "footer-main"].indexOf(targetId) === -1 // Task #1855.2
          ){
            needReturn = true;
        }else 
        if($targetEl.hasClass("search-input") ||
        ...
        if($targetEl.hasClass("relax-start-btn") ||
