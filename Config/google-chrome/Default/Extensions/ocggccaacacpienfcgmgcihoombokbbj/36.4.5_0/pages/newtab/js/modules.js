var PickURL, Preview;
var LocDef = String(localStorage.getItem('definedLocation')).toLowerCase().split('-').shift();
...
function CustomPreview(){
    ...
    this.findMoreIcons =(add)=>{
        ...
        var src = "/pages/newtab/img/common/local-preview.png";
        
        switch(LocDef){
            case "ru": src = "/pages/newtab/img/common/local-preview-ru.png"; break;
            case "de": src = "/pages/newtab/img/common/local-preview-de.png"; break;
            case "it": src = "/pages/newtab/img/common/local-preview-it.png"; break;
            case "fr": src = "/pages/newtab/img/common/local-preview-fr.png"; break;
            case "es": src = "/pages/newtab/img/common/local-preview-es.png"; break;
            case "pt": src = "/pages/newtab/img/common/local-preview-pt.png"; break;
            default: src = "/pages/newtab/img/common/local-preview.png";
        };
    ...
    this.sidePreviewsPrint =(list, page)=> {        
        if(!list || !list.previews || !list.previews.length){
            ...
            if(ts.state.hostCom){ // Moved down Task #865
                //console.info("ts.state.hostCom", ts.state.hostCom, ts.state.tryHostCom);
                ...
                ts.state.tryHostCom = false; // Added for Task #865
            }
        ...
        //console.info(list.previews);
        
        var li = [], printed=0;
        for(var key in list.previews){
            try{
                if(
                    ts.state.printedPreviews.indexOf(list.previews[key].url) == -1
                    &&
                    ui.$prevList.find(".previewImage[src='"+list.previews[key].url+"']").length == 0
    ...
    this.hostDotCom = (host) => {
        ...
        if(ts.state.tryHostCom){
            if(
                !hostArr.length ||
                ts.state.tryHostCom.indexOf(hostArr[0]) === -1
            ){
                ts.sidePreviewsUI("hide");
                ts.state.tryHostCom = false;
            }else{
                return ts.state.tryHostCom;
            }
        }
    ...
    this.getPreview =(host, page)=> {
        ...
        if(checkUrlHasGoogleHost(host, true)){
        ...
        if(ts.enabled){
            ts.GET({
                ...
                success: (data, param)=>{  
                    if(!param.p) ts.state.printedPreviews = [];
                    
                    //console.info("success", data, param);
                    ...
                    if(
                        ...
                        //console.info("Return", param.host, ts.state.host, ts.state.hostCom, param.p, ts.state.page, param.order, ts.state.order);
                        return;
                    }

                    ts.state.totalPages = data.totalPages;

                    //console.info("Previews for '"+host+"' :", data);
