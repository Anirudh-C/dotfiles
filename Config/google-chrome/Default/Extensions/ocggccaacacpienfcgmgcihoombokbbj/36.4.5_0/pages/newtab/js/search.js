    ...
    function addSearchItemToList(searchQuery, $searchContainer, itemVal, isSiteUrl) { 
        ...
        $searchBarItem.attr("data-search-url", isSiteUrl ? addDataUrlHttp(itemVal) : getRedirectSearchUrl(
            itemVal, 
            parseInt(localStorage.getItem('current-search-provider')) || searchProviderLive)
        );
    ...
    $(function() {
            ...
            if(searchInput && searchInput.val()) {
                ...
                if(searchVal) {
                    getSearchFormProviderType(function(searchProvider) {
                        //console.info(searchVal, searchProvider, getRedirectSearchUrl(searchVal, searchProvider)); // Task #1662
        ...
        BRW_langLoaded(function(){BRW_getFileSystem(function(){
            getSearchFormProviderType(function(searchProvider) {
                ...
                localStorage.setItem('current-search-provider', String(searchProvider));
            }, true);
        });});
        ...
        getSearchEngineLabelModeAsync((val)=>{
            if(Boolean(val)){
                ...
                $("#search-suggestion").addClass("search-suggestion-wide");
            }
        });
