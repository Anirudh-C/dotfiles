...
function menuLinksHandler($link, e, target){
    e.preventDefault();
    //e.stopPropagation();
    ...
    if(event.ctrlKey || e.which == 2 || target == "_blank") // Task #2025
        openUrlInNewTab(url);
    else
        openUrlInCurrentTab(url);
}
...
function floatMenuBottom(){
    ...
    $button.on("click", "ul li", function(e){
        ...
        if($(e.target).attr('href')){
            menuLinksHandler($(e.target), e, $(e.target).attr('target') || false); // Task #2025
        }
    });
