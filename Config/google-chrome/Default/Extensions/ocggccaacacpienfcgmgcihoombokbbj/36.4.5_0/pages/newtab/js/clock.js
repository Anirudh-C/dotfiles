...
$(function(){
    ...
    if(parseInt(localStorage.getItem('show-clock-with-dials')) && getVisibleSpeedDialPanel()){
        clockViewMode = "second";
    }
...
function clockScaleHandler(){
    ...
    $clockWrap
        .removeClass("clock-size-small")
        .removeClass("clock-size-medium")
        .removeClass("clock-size-large")
        .addClass("clock-size-" + (clockViewMode == "main" ? size : "small"))
    ;
    
    $clockWrap
        .removeClass('clock-main').removeClass('clock-second')
        .addClass('clock-' + clockViewMode)
    ;
}
...
function loadClockPlaceState(callback){
    getSettingsValue(clockPlaceName(), null, function(data) {
...
var displayClockLazyTrigger = false;
function displayClock(response){//, lazy) {
    /*
    if(lazy){
        if(displayClockLazyTrigger) return;
        else displayClockLazyTrigger = true;
    }
    */
...
function clockPlaceName(){
    var name = clockViewMode != "second" ? "clock-place" : "clock-place-second";
    
    //console.info('clockPlaceName', name, Date.now());
...
function fadeInClock(speed, param){
    //console.info('fadeInClock');
    
    var $clock = $("#clock-container");
    var $clockWrap = $("#mv-clock-wrap");
    
    if(!$clock.is(":visible") || $clock.attr("hmode") == "hidden"){
        $clock.fadeIn(speed || 1600);
        
        $clockWrap.fadeIn(speed || 1600);
        
        $clock.attr("hmode", "visible");
...
function fadeOutClock(speed, param){
    //console.info('fadeOutClock');
    ...
    if($clock.is(":visible") && $clock.attr("hmode") != "hidden") {
        $clock.attr("hmode", "hide");
        FocusLikeClock("hide");
        
        if(VAL.get('show-clock-with-dials')){
        ...
        if(typeof param.onHide == "function"){
            setTimeout(()=>{
                 param.onHide();
            }, 150);
        }
        
        switchClockMode('second');
    } else {
        if(!getDisplayClockPanel()) FocusLikeClock("hide");
        
        if(typeof param.onShow == "function") param.onShow();
...
var clockModeTimer1 = false;
var clockModeTimer2 = false;
function switchClockMode(mode){
    //console.info('switchClockMode', mode, Date.now());
    
    if(mode == clockViewMode) return false;
    
    clockPlaceState = false;
    ...
    if(VAL.get('show-clock-with-dials')){
        ...
        $clock.stop(true).fadeOut(150);

        clearTimeout(clockModeTimer1);
        clearTimeout(clockModeTimer2);
        
        clockModeTimer1 = setTimeout(()=>{
            clockScaleHandler();
            loadClockPlaceState();
            
            clockModeTimer2 = setTimeout(()=>{
                $clock.stop(true).fadeIn(150);
                addClockBlockDraggableProperty();
                clockAlignment();
            }, 350);
        ...
        if(lastDatePrint != dateStr){
            ...
            if(!$el.length){
                ...
                if(!$wr.length){
                    ...
                    if($('.clock-format-label').length){
                        $el.addClass('date-and-label');
                        $(".ce-countdown").addClass("ce-add-label");
                    }
...
function addClockBlockDraggableProperty() { 
    ...
    if($wrap.hasClass('ui-draggable')){
        $wrap.draggable( "destroy" );
...
function updateClockCoordinates(){
    ...
    getSettingsValue(clockPlaceName(), null, function(data) {
        ...
        setSettingsValue(clockPlaceName(), JSON.stringify(data), function(data2) {
            clockAlignment();
        });
    });
...
function getClockPlace(safe, align){
    ...
    if(!clockPlaceState || typeof clockPlaceState.top == "undefined"){
        if(clockViewMode !== "second"){//main
            clock.top  = Math.max(Math.floor(clock.win.h / 2) - 50, (50 + clock.h/2));
        }else{
            clock.top  = 83;
        }
    ...
    if(!clockPlaceState || typeof clockPlaceState.left == "undefined"){
        if(clockViewMode !== "second"){//main
            ...
            if(getClockFormat() && getClockFormatLabel()){// shift for AM/PM
                var shift = parseInt($("#clock-format-label").outerWidth()) || 0;
                if(shift) clock.left += shift / 2;
            }
        }else{
            var left = clock.win.w - 150 - 285;
            
            if(clock.win.w <= 1430) left += 70;
            else
            if(clock.win.w <= 1650) left += 60;
            else
            if(clock.win.w <= 1800) left += 50;
            ...
            /*
            try{
                var wrapH = $('.sidebar-content-scroll').innerHeight();
                var groups = 1 + $('.sidebar-group-item').length;
                
                if(wrapH - 50 * groups < 155){
                    left -= 240;
                }
    ...
    //console.info('getClockPlace', clockPlaceName(), clock);
